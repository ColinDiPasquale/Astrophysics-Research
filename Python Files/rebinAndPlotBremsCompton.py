...
def plot_two_spectra(bin_edges_brems, counts_brems, widths_brems,
    ...
    plt.step(bin_edges_compton[:-1], counts_per_width_compton, where='post', color='blue', label='Comptonized Photons')
    ...
    plt.title('Bremsstrahlung and Comptonized Photons at 40 days')
    plt.grid(True, which='both', ls='--', lw=0.5)
    plt.legend()
    plt.tight_layout()
    plt.savefig('../bremsAndCompt.png', dpi=300)
    print("Plot saved as 'bremsAndCompt.png'")
...
def main():
    ...
    compton_lower, compton_upper, compton_counts = read_spectrum_data('All_compton_spectrum_combined.txt')
